...
export default function DepositPage() {
    ...
    const handlePercent = (p: number) => {
        setPercent(p);
        setAmount((((tonBalance - 0.3) * p) / 100).toFixed(2));
    };

    // 处理质押
    const handleDeposit = async () => {
        console.log('用户质押金额：' + amount);
        if (!userAddress) {
            alert("请先连接您的钱包！");
            return;
        }
        ...
        const message = beginCell()
            .storeUint(10086, 32)
            .storeAddress(Address.parse(userAddress)) // 存款人
            .storeUint(toNano(tonAmount), 64)
            .endCell();
        console.log("整体交易费用"+ (0.3 + tonAmount))
        // 创建交易并发送
            ...
            messages: [
                {
                    ...
                    amount: toNano(0.3 + tonAmount).toString(),  // 交易费用
    ...
    return (
        <div style={{background: '#fff', borderRadius: 18, margin: 24, padding: 24, maxWidth: 480, minHeight: 600}}>
            ...
            <div style={{marginTop: 24}}>
                <InfoRow label="MAX" value={`${(tonBalance - 0.3)} ${asset.name}`} icon={asset.icon}/>
